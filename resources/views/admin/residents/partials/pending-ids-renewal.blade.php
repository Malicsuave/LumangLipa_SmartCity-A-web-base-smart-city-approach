    ...
    <div class="card-body">
        <div class="table-responsive">
            @if($pendingRenewal->count() > 0)
                <table class="table table-bordered table-striped" id="renewalTable" data-export-title="Pending ID Renewal">
                    <thead>
                        <tr>
                            <th>Barangay ID</th>
                            <th>Name</th>
                    ...
                    <tbody>
                        @foreach($pendingRenewal as $resident)
                            <tr>
                                ...
                                <td>
                                    @if($resident->expected_renewal_date)
                                        {{ $resident->expected_renewal_date->format('M d, Y') }}
                                        <br><small class="text-muted">
                                            @if($resident->id_expires_at && $resident->id_expires_at->isPast())
                                                Expired
                                            @elseif($resident->renewal_date)
                                                Marked for renewal
                                            @else
                                                Expiry date
                                            @endif
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <h4>No residents pending renewal</h4>
