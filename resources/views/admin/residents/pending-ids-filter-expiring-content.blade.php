{{-- Expiring Soon Filter Form, Active Filters, and Table --}}
<form action="{{ route('admin.residents.id.pending') }}" method="GET" id="expiringFilterForm">
    <input type="hidden" name="tab" value="expiring">
    <div class="row mb-4">
        <div class="col-md-8">
            <div class="input-group">
                <input type="text" class="form-control" name="expiring_search" placeholder="Search by name, Barangay ID, phone number..." value="{{ request('expiring_search') }}">
        ...
        <div class="col-md-4">
            @if(request()->hasAny(['expiring_search', 'expiring_type', 'expiring_gender', 'expiring_age_group']))
                <a href="{{ route('admin.residents.id.pending') }}?tab=expiring" class="btn btn-outline-secondary">
    ...
    <div class="collapse {{ request()->hasAny(['expiring_type', 'expiring_gender', 'expiring_age_group']) ? 'show' : '' }}" id="expiringFilterSection">
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="expiring_type" class="form-control form-control-sm">
                            ...
                            <option value="Non-migrant" {{ request('expiring_type') == 'Non-migrant' ? 'selected' : '' }}>Non-migrant</option>
                            <option value="Migrant" {{ request('expiring_type') == 'Migrant' ? 'selected' : '' }}>Migrant</option>
                            <option value="Transient" {{ request('expiring_type') == 'Transient' ? 'selected' : '' }}>Transient</option>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="expiring_gender" class="form-control form-control-sm">
                            ...
                            <option value="Male" {{ request('expiring_gender') == 'Male' ? 'selected' : '' }}>Male</option>
                            <option value="Female" {{ request('expiring_gender') == 'Female' ? 'selected' : '' }}>Female</option>
                            <option value="Non-binary" {{ request('expiring_gender') == 'Non-binary' ? 'selected' : '' }}>Non-binary</option>
                            <option value="Transgender" {{ request('expiring_gender') == 'Transgender' ? 'selected' : '' }}>Transgender</option>
                            <option value="Other" {{ request('expiring_gender') == 'Other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="expiring_age_group" class="form-control form-control-sm">
                            ...
                            <option value="0-17" {{ request('expiring_age_group') == '0-17' ? 'selected' : '' }}>0-17 (Minor)</option>
                            <option value="18-59" {{ request('expiring_age_group') == '18-59' ? 'selected' : '' }}>18-59 (Adult)</option>
                            <option value="60+" {{ request('expiring_age_group') == '60+' ? 'selected' : '' }}>60+ (Senior)</option>
                        </select>
                    </div>
                </div>
                <div class="row mt-4">
        ...
        <div class="mb-3">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <span class="badge badge-info ml-2">{{ $expiringSoon->total() }} results found</span>
                </div>
                <small class="text-muted">Click on any filter badge to remove it</small>
            </div>
            <div class="mt-2">
                @if(request('expiring_search'))
                    <a href="{{ request()->fullUrlWithQuery(['expiring_search' => null, 'tab' => 'expiring']) }}" class="badge badge-dark mr-1 text-decoration-none">
                        Search: {{ request('expiring_search') }} <i class="fe fe-x"></i>
                    </a>
                @endif
                @if(request('expiring_type'))
                    <a href="{{ request()->fullUrlWithQuery(['expiring_type' => null, 'tab' => 'expiring']) }}" class="badge badge-primary mr-1 text-decoration-none">
                        Type: {{ request('expiring_type') }} <i class="fe fe-x"></i>
                    </a>
                @endif
                @if(request('expiring_gender'))
                    <a href="{{ request()->fullUrlWithQuery(['expiring_gender' => null, 'tab' => 'expiring']) }}" class="badge badge-success mr-1 text-decoration-none">
                        Gender: {{ request('expiring_gender') }} <i class="fe fe-x"></i>
                    </a>
                @endif
                @if(request('expiring_age_group'))
                    <a href="{{ request()->fullUrlWithQuery(['expiring_age_group' => null, 'tab' => 'expiring']) }}" class="badge badge-info mr-1 text-decoration-none">
                        Age Group: {{ request('expiring_age_group') }} <i class="fe fe-x"></i>
                    </a>
                @endif
            </div>
        </div>
